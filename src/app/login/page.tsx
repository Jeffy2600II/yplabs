  ...
  async function handleStudentLogin(e: React.FormEvent) {
    ...
    try {
      ...
      const { data: row, error: rowErr } = await supabase
        .from('council_users')
        .select('full_name,approved,disabled,account_type')
      ...
      // refresh AuthContext ก่อน push เพื่อให้หน้าปลายทางได้ user state ทันที
      await refresh();
      router.push('/');
    } catch (err: any) {
            ...
            <form onSubmit={handleStudentLogin} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div className="form-group">
                ...
                <input value={fullName} onChange={e => { setFullName(e.target.value); setError(null); }}
                  placeholder="เช่น สมชาย ใจดี" required autoFocus disabled={loading} />
              </div>
              <div className="form-group">
                <label className="form-label">รหัสนักเรียน (5 หลัก)</label>
                <input value={studentId} onChange={e => { setStudentId(e.target.value); setError(null); }}
                  placeholder="12345" inputMode="numeric" maxLength={5} required disabled={loading} />
              </div>
            ...
            <form onSubmit={handleOtherLogin} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div className="form-group">
                ...
                <input type="email" value={email} onChange={e => { setEmail(e.target.value); setError(null); }}
                  placeholder="teacher@school.ac.th" required autoFocus disabled={loading} />
              </div>
              <div className="form-group">

...
import { xhrWithAuth } from '@/lib/sessionUtils';
...
export default function SubmitPage() {
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (f && f.size > 5 * 1024 * 1024) {
      void remoteLog('warn', '[submit] file too large', { name: f.name, size: f.size });
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    setLoading(true); setProgress(null); setError(null); abortRef.current = false;
    ...
    try {
      // ★ xhrWithAuth: token refresh อัตโนมัติ + retry 401 อัตโนมัติ
      await xhrWithAuth({
        url: '/api/council/submit',
        body: fd,
        onProgress: pct => { if (!abortRef.current) setProgress(pct); },
      });

      void remoteLog('info', '[submit] OK', { title, user: user?.full_name });
      setDone(true); setFileName(''); setProgress(null);

    } catch (err: any) {
      if (!abortRef.current) setError(err?.message ?? 'เกิดข้อผิดพลาด');
    } finally {
      setLoading(false);
    }
  }

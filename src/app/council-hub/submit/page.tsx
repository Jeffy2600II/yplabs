  ...
  async function handleSubmit(e: any) {
    ...
    try {
      ...
      if (res?.data?.success) {
        ...
        // non-200 but resolved? treat as error
        setToast({ type: "error", title: "เกิดข้อผิดพลาด", message: res?.data?.error ?? "ไม่ทราบสาเหตุ", details: res?.data });
      }
    } catch (err: any) {
      setLoading(false);
      setProgress(null);
      ...
      if (err?.data) {
        setToast({ type: "error", title: err.data.error ?? "อัปโหลดล้มเหลว", message: err.data.message ?? undefined, details: err.data.details ?? err.data });
      } else {
        ...
        <div role="dialog" aria-live="assertive" style={{ position: "fixed", left: "50%", transform: "translateX(-50%)", bottom: 32, minWidth: 280, maxWidth: "90%", background: toast.type === "success" ? "#0b6623" : "#7a1f1f", color: "#fff", padding: "12px 16px", borderRadius: 8, boxShadow: "0 6px 24px rgba(0,0,0,0.5)", zIndex: 9999 }}>
          ...
          {toast.details && (
            <div style={{ marginTop: 8 }}>
              <button onClick={() => setShowDetails(s => !s)} style={{ border: "none", background: "transparent", color: "#fff", textDecoration: "underline", cursor: "pointer" }}>
                {showDetails ? "ซ่อนรายละเอียดแสดงข้อผิดพลาด" : "ดูรายละเอียดข้อผิดพลาด (สำหรับนักพัฒนา)"}
              </button>
              {showDetails && (

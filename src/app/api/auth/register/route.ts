  ...
  try {
    ...
    if (account_type === 'student') {
      ...
      if (dupErr) {
        ...
        // ไม่ block — ให้ผ่านไปก่อน แล้ว insert จะ fail เองถ้า duplicate
      }
      ...
      logger.debug('inserting student join request', {
        student_id,
        year,
        synEmail: synthesized,
      });
      ...
      if (error) {
        logger.supabaseError('insert council_join_requests (student)', error, {
          student_id,
          year,
        });
        throw error;

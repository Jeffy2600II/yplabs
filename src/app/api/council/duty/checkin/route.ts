...
export async function POST(req: NextRequest) {
  ...
  if (findErr) {
    logger.supabaseError('lookup duty entry', findErr, { uid: member.id.slice(-6), date: today });
  ...
  if (updateErr) {
    logger.supabaseError('update duty check-in', updateErr, { entryId: entry.id, uid: member.id.slice(-6) });
